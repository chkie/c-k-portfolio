...
<section data-slice-type={slice.slice_type} data-slice-variation={slice.variation} class="px-4 md:px-6">
  <div class="mx-auto w-full max-w-7xl">
    <div class="grid min-h-[65vh] grid-cols-1 items-center md:grid-cols-2">
      <div class="relative z-10 row-span-1 row-start-1 -my-10 aspect-[1/1.3] cursor-pointer overflow-hidden md:col-span-1 md:col-start-2 md:mt-0">
        <Scene />
      </div>
      <div class="col-start-1 md:row-start-1">
        <h1 class="mb-2 md:mb-2 text-[clamp(3rem,20vmin,11rem)] font-extrabold leading-none tracking-tight text-nowrap" aria-label={slice.primary.first_name + ' ' + slice.primary.last_name}>
          {#if first_name_letters.length && last_name_letters.length}
            <span class="block text-slate-300 mb-[.2em]">
        ...
        <span class="ml-2 tag-line block bg-gradient-to-tr from-yellow-500 via-yellow-200 to-yellow-500 bg-clip-text uppercase text-transparent text-2xl font-bold tracking-[.2em] md:text-4xl opacity-0">
